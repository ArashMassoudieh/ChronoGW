...
//		filename = "Input_IG_old_age_oneWell.txt";
	...
	Conc.resize(50); //Number of years ,
	...
	{
		int y = (year - 1900) / 10;
		Conc[y].resize(system.Tracer.size());
		cout << "\n" + std::to_string(year) +"\n";


		for (int k = 0; k < system.Tracer.size(); k++)
			Conc[y][k] = CMatrix(ni, nj);


		for (int i = 0; i < ni; i++)
		{
			cout << std::to_string((100 * i) / ni) + "% ";
			float mu = exp(log(min_mu) + log(max_mu / min_mu) / ni*(i + 1));
			for (int j = 0; j < nj; j++)
			{
				float lambda = exp(log(min_lambda) + log(max_lambda / min_lambda) / nj*(j + 1));
				...
				for (int k = 0; k < system.Tracer.size(); k++)
					Conc[y][k][i][j] = system.Tracer[k].calc_conc(year, dist, 0);
			}
		}

		for (int k = 0; k < system.Tracer.size(); k++)
			Conc[y][k].writetofile("out/tracers/" + system.Tracer[k].name + "-" + std::to_string(year) + ".txt");
	}
	return 0;
}
