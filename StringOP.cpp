...
CStringOP::CStringOP(string S)
{
	...
	*/
	...
	for (int i=0; i<S.size(); i++)
	{
		...
		if ((i<S.size()) && (S[min(i,(int)S.size()-1)]=='p'))
		{
			int j=i+1;
			while (S[j] != ']') j++;
			{
				terms.push_back(CStringOP());	
				terms[terms_count].number = atoi(S.substr(i+2,S.size()-1).c_str());
				terms_count++;
		...
		if ((i<S.size()) && (S[min(i,(int)S.size()-1)]=='c'))
		{
			...
			{
				terms.push_back(CStringOP());	
				terms[terms_count].number = ATOI(split(S.substr(i+2,S.size()-1),','))[0];
...
vector<string> split(string s, char del=',') //Returns a string vector separated by "," ??? "="
{
	int lastdel=0;
	...
	for (int i=0; i<s.size(); i++)   
	{
		if (s[i]==del)
		{
			strings.push_back(s.substr(lastdel, i-lastdel));
			lastdel = i+1;
			j++;		// j is Not Used- can be deleted
		}
	}
